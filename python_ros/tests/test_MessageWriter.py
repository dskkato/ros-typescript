...
"""
    ...
    expected = b"\x00\x01\x00\x00" + struct.pack("<i", -42) + _serialize_string("hello")
...
"""
    ...
    expected = b"\x00\x01\x00\x00" + struct.pack("<I", 3) + struct.pack("<3i", 1, 2, 3)
    assert writer.write_message(message) == expected
...
"""
    defs = parse(msg_def, ros2=True)
    writer = MessageWriter(defs)
    message = {"stamp": {"sec": 1, "nanosec": 2}}
    expected = b"\x00\x01\x00\x00" + struct.pack("<iI", 1, 2)
    assert writer.write_message(message) == expected
...
module builtin_interfaces {
  struct Time {
    ...
    assert writer.write_message(message) == buffer
